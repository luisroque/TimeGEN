...
def plot_series_comparisons(X_orig, X_hitgen, X_benchmark, n_examples=4):
    """
    ...
    - X_hitgen: numpy.ndarray, the dataset generated by L-GTA transformation.
...
def plot_transformations_with_generate_datasets(
    ...
    for idx_t, transformation_info in enumerate(transformations):
        ...
        for idx_s in range(num_series):
            ...
            axs[idx_s, col_hitgen].plot(
                X_orig_plot[:, idx_s], label="Original", color="gray", linestyle="--"
            )
            axs[idx_s, col_hitgen].plot(
                X_hat_transf[:, idx_s], label="L-GTA", color="darkorange", alpha=0.75
            )
            axs[idx_s, col_hitgen].set_ylim(global_min, global_max)
            axs[idx_s, col_hitgen].legend()
            axs[idx_s, col_hitgen].grid(True, linestyle="--", linewidth=0.5)
            ...
            # Setting titles
            if idx_s == 0:
                axs[idx_s, col_hitgen].set_title(
