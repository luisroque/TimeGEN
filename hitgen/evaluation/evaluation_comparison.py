...
def plot_residuals(residuals_hitgen, residuals_benchmark, n_series=4):
    """
    Plots the residuals of L-GTA and benchmark transformations side by side
    for the first n time series.

    Parameters:
    - residuals_hitgen: Residuals from the L-GTA transformation.
    ...
    for i in range(n_series):
        axs[i, 0].plot(residuals_hitgen[i], label="L-GTA Residuals", color="darkblue")
        axs[i, 1].plot(
...
def analyze_transformations(residuals_hitgen, residuals_benchmark):
    plot_residuals(residuals_hitgen, residuals_benchmark)
    # Flatten the 2D residuals arrays
    flat_residuals_hitgen = residuals_hitgen.flatten()
    flat_residuals_benchmark = residuals_benchmark.flatten()

    # Calculating mean and std for both methods
    mean_hitgen, std_hitgen = np.mean(flat_residuals_hitgen), np.std(flat_residuals_hitgen)
    ...
    sns.kdeplot(flat_residuals_hitgen, label="hitgen", color="skyblue")
    sns.kdeplot(flat_residuals_benchmark, label="Direct", color="lightgreen")

    # Marking mean and std
    plt.axvline(mean_hitgen, color="blue", linestyle="--", label="Mean - hitgen")
    plt.axvline(mean_benchmark, color="green", linestyle="--", label="Mean - Direct")

    plt.fill_betweenx(
        [0, plt.ylim()[1]],
        mean_hitgen - std_hitgen,
        mean_hitgen + std_hitgen,
        color="blue",
        alpha=0.2,
        label="Std Dev - hitgen",
    )
    plt.fill_betweenx(
        [0, plt.ylim()[1]],
    ...
    x_min = min(mean_hitgen - 3 * std_hitgen, mean_benchmark - 3 * std_benchmark)
    x_max = max(mean_hitgen + 3 * std_hitgen, mean_benchmark + 3 * std_benchmark)
    ...
    # Variance Analysis
    variance_hitgen = np.var(residuals_hitgen, axis=1)
    variance_benchmark = np.var(residuals_benchmark, axis=1)
    print(
        f"Mean Variance - hitgen: {np.mean(variance_hitgen)}, Benchmark: {np.mean(variance_benchmark)}"
    )
