...
class CreateGroups:
    def __init__(self, dataset_name: str, freq: str, sample_perc: float = None):
        ...
        self.freq = freq
        self.sample_int_perc = (
            int(sample_perc * 100) if sample_perc is not None else None
        )
    ...
    def _preprocess_dataset(self, sample_perc: float = None) -> dict:
        ...
        dataset = PreprocessDatasets(
            self.dataset_name, sample_perc=sample_perc, freq=self.freq
        )
