...
def compute_and_print_errors(X_orig, X_hat_transf, X_benchmark):
    ...
    print(f"Reconstruction Error (L-GTA): {reconstruction_error_hitgen:.4f}")
    print(f"Reconstruction Error (Benchmark): {reconstruction_error_benchmark:.4f}\n")

    print(
        f"KL Divergence (L-GTA): Median = {kl_div_hitgen_median[0]:.4f}, IQR = {kl_div_hitgen_iqr[0]:.4f}"
    )
    print(
    ...
    return (
        ...
        (
            reconstruction_error_hitgen,
            kl_div_hitgen_median[0],
            kl_div_hitgen_iqr[0],
            kl_div_hitgen_skew[0],
            kl_div_hitgen_kurtosis,
        ),
        (
            reconstruction_error_benchmark,
...
def summarize_and_plot_distributions(distances_hitgen, distances_benchmark, title):
    # Calculate metrics
    metrics_hitgen = {
        "Mean": np.mean(distances_hitgen),
        "Median": np.median(distances_hitgen),
        "IQR": iqr(distances_hitgen),
        "Skewness": skew(distances_hitgen),
        "Kurtosis": kurtosis(distances_hitgen),
    }
    ...
    # Box plots
    plt.subplot(1, 2, 2)
    data_to_plot = [distances_hitgen, distances_benchmark]
