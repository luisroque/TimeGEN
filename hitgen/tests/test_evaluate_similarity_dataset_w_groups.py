import unittest
import numpy as np
from hitgen.transformations.compute_similarities_summary_metrics import (
    ComputeSimilaritiesSummaryMetrics,
)
...
class TestEvaluateSimilarityWithGroups(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.transf_dataset = np.random.normal(
            0, 2, size=(self.n_points, self.n_series)
        )
        self.groups = 3
        self.groups_idx = {
            "group1": np.arange(self.n_series) % 3,
    ...
    def test_evaluate_similarity_within_specific_group(self):
        ...
        compute_similarities = ComputeSimilaritiesSummaryMetrics(
            self.base_dataset, self.groups_idx
        ).compute_similarities_within_group(group, group_val)
        self.assertIsNone(
            np.testing.assert_almost_equal(similarities, compute_similarities)
        )

    def test_evaluate_similarity_for_every_specific_group_element(self):
        similarities = {
            "group1": [10.624993776985308, 11.397565415605204, 10.451836642651857],
