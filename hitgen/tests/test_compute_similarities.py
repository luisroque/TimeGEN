import unittest
import numpy as np
from hitgen.transformations.compute_similarities import ComputeSimilarities
from hitgen.transformations.similarity_model import SimilarityModel
...
class TestComputeSimilarities(unittest.TestCase):
    ...
    def test_compute_similarities_between_pairs_of_ts_of_dataset_with_one_series_different(
        self,
    ):
        k = 0
        selected_series = 4
        ...
        sim_diff_series_value = similarity_model.estimate_similarity_two_series(
            np.arange(self.n_points), np.cos(np.arange(self.n_points))
        )
    ...
    def test_compute_avg_similarity_between_single_series_and_all_series_in_a_dataset(
        self,
    ):
        avg_sim = ComputeSimilarities(
            self.base_dataset, self.transformed_series
    ...
    def test_mean_similarity_elementwise_with_same_dataset(self):
        avg_sim = ComputeSimilarities(
            dataset=self.base_dataset, transf_dataset=self.base_dataset
