...
def create_dynamic_features(df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    for period in periods:
        if period == "week":
            max_val = df.index.isocalendar().week.max()
            min_val = df.index.isocalendar().week.min()
            current_val = df.index.isocalendar().week
        else:
            max_val = getattr(df.index, period).max()
            min_val = getattr(df.index, period).min()
            current_val = getattr(df.index, period)

        if (max_val - min_val) > 0:
            train_df_input[f"{period}_cos"] = np.cos(
                2 * np.pi * (current_val - min_val) / (max_val - min_val)
            )
