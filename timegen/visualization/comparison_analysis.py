...
def plot_series_comparisons(X_orig, X_timegen, X_benchmark, n_examples=4):
    """
    Plots comparisons between original, L-GTA generated, and benchmark generated series, styled similarly to plot_long_tail_comparisons.

    Parameters:
    - X_orig: numpy.ndarray, the original dataset.
    - X_timegen: numpy.ndarray, the dataset generated by L-GTA transformation.
    - X_benchmark: numpy.ndarray, the dataset generated by benchmark transformation.
    - n_examples: int, the number of examples to plot.
    """
    ...
    colors = sns.color_palette("husl", 3)  # Use a consistent color palette
    ...
    fig.suptitle(
        "Comparisons: Original vs Transformed Series",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def plot_transformations_with_generate_datasets(
    ...
    for idx_t, transformation_info in enumerate(transformations):
        ...
        for idx_s in range(num_series):
            ...
            axs[idx_s, col_timegen].plot(
                X_orig_plot[:, idx_s], label="Original", color="gray", linestyle="--"
            )
            axs[idx_s, col_timegen].plot(
                X_hat_transf[:, idx_s], label="L-GTA", color="darkorange", alpha=0.75
            )
            axs[idx_s, col_timegen].set_ylim(global_min, global_max)
            axs[idx_s, col_timegen].legend()
            axs[idx_s, col_timegen].grid(True, linestyle="--", linewidth=0.5)
            ...
            # Setting titles
            if idx_s == 0:
                axs[idx_s, col_timegen].set_title(
